...
#include "AD9833.h"
#include "delay.h"
//PE5 SSI1XDAT1 PE4 SSI1XDAT0  PB5  SSI1Clk      PB4   SSI1Fss
...
void AD9833_WaveSeting(double Freq,unsigned int Phase,unsigned int Freq_SFR,unsigned int Freq_SPR)
{
		...
		frequence_LSB=frequence_hex&0x3fff; //frequence_hex低16位送给frequence_LSB
		frequence_LSB=frequence_LSB;//去除最高两位，16位数换去掉高位后变成了14位
		...
		{
		 	AD9833_Write(0xC000|Phase);
	  }else if(Freq_SPR==1){
			AD9833_Write(0xD000|Phase);
		}
